...
- まりもは生後日数が1日増えるたびに `0.3 mm` ずつ、上限なく成長する。同じ生後日数なら同じ大きさになる。
- 日本時間で1日1回だけ水を替えられ、既定では `100 XP` を獲得する。
...
- 大きさランキングには生存日数も併記し、水替え・死亡のたびに同じ常設投稿を編集する。

## Discord コマンド

一般ユーザーは水替えパネルの「育て始める・水を替える」「自分のまりも」「名前をつける」ボタンだけで操作する。初回は自分のまりもが生まれ、以後は同じボタンで1日1回水を替える。
