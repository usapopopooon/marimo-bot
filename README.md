...
- 枯れた後は、次回の水替えで新しい世代を始めるか、`1,000 XP` または「カフェ・コレクション」で手に入る「苔コーラ」の重複分1本で同じまりもを復活できる。苔コーラはコレクションに残す最初の1本を消費しない。
- 死亡ログには、持ち主にも他のユーザーにも同じ「1,000 XPで復活」「苔コーラを与える」を表示する。押した人のXPまたは苔コーラを使い、最初に確定した1人だけが消費する。苔コーラは消費前に押した人だけへ確認を表示する。古い死亡ログや復活済みのまりもへの操作では消費しない。持ち主が次の世代を育て始めたら旧世代は復活対象外となり、救助ボタンと案内も表示しない。
- 復活したまりもは世代・名前・大きさを引き継ぎ、枯れていた期間は成長日数に含めない。復活当日は水替え済みとして扱う。死亡ログは死亡時の記録として変更せず、復活時は別の新規投稿として、生き返った飼育画面と、持ち主・助けた人・使ったXPまたは苔コーラをログへ公開する。
- 水替え・死亡・復活時には、設定したログチャンネルへ現在の飼育画面画像を投稿する。
- 枯れたときは深夜0時の通知音を避けるため、本人の表示名を通常テキストで記録し、メンションは表示も通知もしない。連続飼育2日・3日・5日と10日以降10日ごとの水替えでは、ログ末尾で達成を祝って本人へ通知する。
- 水替え通知は利用者ごとにデフォルトOFFとし、希望者だけが日本時間の8時・12時・18時・21時から選べる。指定時刻を過ぎても当日の水替えがまだなら、設定済みのまりもログで本人だけを1日1回メンションする。水替え済みまたは枯れている場合は通知せず、送信失敗時の再試行も同日最大5回で停止する。
- 新しい水替えログでは、誕生・飼育日数・大きさ・季節・日本時間の時間帯に合う1,400種類のセリフから、無害で少し怪しい水槽内の出来事と、本人は気にしていない軽い自虐を中心にしたまりものひとことを「名前「セリフ」」の形で表示する。22時から翌4時59分までは、誕生・連続記録の専用セリフを除き、静かで癒やし寄りの深夜セリフを使う。利用者を採点・指示・揶揄する口調は使わない。同じ利用者の直近7件とは文章だけでなく同じ前半・オチもできるだけ重複させず、選んだセリフは水替え記録へ保存する。
...
一般ユーザーは埋め込み形式の水替えパネルにある「育て始める・水を替える」「自分のまりもを見る」「名前をつける」「水換え通知」と2種類の復活ボタンで操作する。初回は自分のまりもが生まれ、以後は同じボタンで1日1回水を替える。「自分のまりもを見る」では、現在の水槽画像・飼育情報・その日に水替えしたまりものセリフを、名付けた名前を話者にしてパネルのチャンネル内で本人だけに表示する。「水換え通知」では通知のON/OFFと時刻を本人だけに表示して変更できる。大きさランキングも埋め込み形式で常設する。
...
- `/marimo-admin log` — 実行チャンネルを画像ログと水替え通知の投稿先に設定
- `/marimo-admin log-refresh` — 実行チャンネルの旧画像ログを、DBに残る全水替え・死亡・復活履歴から時系列で作り直す
- `/marimo-admin log-disable` — 画像ログと水替え通知を停止
