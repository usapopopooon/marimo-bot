...
一般ユーザーは埋め込み形式の水替えパネルにある「育て始める・水を替える」「自分のまりもを見る」「名前をつける」「水換え通知」ボタンだけで操作する。初回は自分のまりもが生まれ、以後は同じボタンで1日1回水を替える。「自分のまりもを見る」では、現在の水槽画像・飼育情報・その日に水替えしたまりものセリフを、名付けた名前を話者にしてパネルのチャンネル内で本人だけに表示する。「水換え通知」では通知のON/OFFと時刻を本人だけに表示して変更できる。大きさランキングも埋め込み形式で常設する。
