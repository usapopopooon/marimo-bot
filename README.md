...
- 新しい水替えログでは、誕生・飼育日数・大きさ・季節に合う1,000種類のセリフから、自己ボケや水槽内の小ネタを中心にしたまりものひとことを「名前「セリフ」」の形で表示する。利用者を採点・指示・揶揄する口調は使わない。同じ利用者の直近7件とは文章だけでなく同じ前半・オチもできるだけ重複させず、選んだセリフは水替え記録へ保存する。
...
一般ユーザーは埋め込み形式の水替えパネルにある「育て始める・水を替える」「自分のまりもを見る」「名前をつける」ボタンだけで操作する。初回は自分のまりもが生まれ、以後は同じボタンで1日1回水を替える。「自分のまりもを見る」では、現在の水槽画像・飼育情報・その日に水替えしたまりものセリフを、名付けた名前を話者にしてパネルのチャンネル内で本人だけに表示する。大きさランキングも埋め込み形式で常設する。
...
- `/marimo-admin panel type:枯れたまりもランキング` — 枯れたまりもの常設ランキングを投稿
